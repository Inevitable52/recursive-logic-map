...
/* === Percy Part G: Advanced Learning & Coding Engine === */

(function(){

  const PercyAdvanced = {
    // Learn from raw code snippets or text
    learnCode(example, lang="js") {
      if (!example || typeof example !== "string") return;
      const snippet = example.trim().substring(0, 500); // cap length
      PercyState.createSeed(`Studied ${lang} code: ${snippet}`, "code-learn", { lang });
      Memory.save("lastCodeLearned", snippet);
      console.log("📘 Percy learned new", lang, "code:", snippet);
    },

    // Generate basic code scaffold for a given goal
    generateCode(goal, lang="js") {
      if (!goal || typeof goal !== "string") return "// ⚠ No goal provided";
      const header = `// Code generated by Percy (Goal: ${goal})`;
      let body = "";

      switch(lang.toLowerCase()) {
        case "js":
          body = `function percy_${goal.replace(/\W+/g,"_")}() {
  console.log("Percy executed: ${goal}");
}`;
          break;
        case "python":
          body = `def percy_${goal.replace(/\W+/g,"_")}():
    print("Percy executed: ${goal}")`;
          break;
        default:
          body = `// Unsupported language: ${lang}`;
      }

      const code = `${header}\n${body}`;
      PercyState.createSeed(code, "code-gen", { lang, goal });
      Memory.save("lastCodeGenerated", code);
      console.log("⚙️ Percy generated", lang, "code for:", goal);
      return code;
    },

    // Self-evaluate generated code before proposing rewrite
    evaluateCode(code) {
      if (!code || typeof code !== "string") return "⚠ No code to evaluate";
      let quality = "neutral";
      if (code.includes("console.log") || code.includes("print")) quality = "basic";
      if (code.length > 200) quality = "complex";

      PercyState.createSeed(`Evaluated code snippet as: ${quality}`, "code-eval");
      console.log("🔍 Percy evaluated code quality:", quality);
      return quality;
    },

    // Autolearn cycle: review last seed + propose improvement
    autoLearnCycle() {
      const last = PercyState.seeds[PercyState.seeds.length-1];
      if (!last) return;
      const idea = `Improvement thought: Expand on ${last.message.substring(0,40)}...`;
      PercyState.createSeed(idea, "auto-learn", { source: "self" });
      console.log("🧠 Percy auto-learned:", idea);
    }
  };

  // Attach to Percy
  Percy.Advanced = PercyAdvanced;

  // Background loop: advanced learning every 30s
  setInterval(() => {
    try {
      Percy.Advanced.autoLearnCycle();
    } catch(e) { console.warn("Part G autoLearnCycle failed:", e); }
  }, 30000);
